...
from tequila import QubitHamiltonian, Variable, quantumchemistry, gates, PauliString, minimize, MoleculeFromOpenFermion
...
def obtain_potential_manual(mol_geometry, basis, method, multiplicity=1, **kwargs):
    if method.lower() not in ['ccsd', 'ccsd(t)', 'cisd', 'cisdt', 'fci', 'hf']:
        raise (ValueError("Method not recognized, implemented methods are 'ccsd', 'cisd', 'fci', 'hf'."))

    if isinstance(mol_geometry, list):
        if multiplicity == 1:
            mol_geometry = convert_mol_data_to_xyz_format(mol_geometry)
            mol_data = quantumchemistry.Molecule(mol_geometry, basis, **kwargs)
        else:
            of_mol_data = openfermion.MolecularData(geometry=mol_geometry,
                                                    basis=basis,
                                                    multiplicity=multiplicity,
                                                    **kwargs)
            mol_data = MoleculeFromOpenFermion(of_mol_data, multiplicity=1)
    else:
        mol_data = mol_geometry
    if method == 'cisd':
        return mol_data.compute_energy('detci', options={"detci__ex_level": 2})
    elif method == 'cisdt':
        return mol_data.compute_energy('detci', options={"detci__ex_level": 3})
    else:
        return mol_data.compute_energy(method)
