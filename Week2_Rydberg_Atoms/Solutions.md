...
cost function, and calculated its eigenvalues and eigenvectors. The solutions are codified in the eigenvectors of the ground state eigenspace. In order to extract the solutions, we computed $n_i=\langle Gs(k)| n_i|Gs(k)\rangle$, were $k$ is the degeneracy index.
